...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
  ...
  <script src="jquery.js" type="text/javascript"></script>
	...
	if(isset($_SESSION['admin']) && $_SESSION['admin']==1)
		echo '<li id="a1"><a href="Add_admin.php">Add admin</a></li><li id="a1"><a href="Add_to_home.php">All users</a></li>';
	if(isset($_SESSION['email'])){
  ...
  $search=$_GET['pl'];
	...
	$s="select * from bloggers where name='$search'";
			$res=$conn->query($s);

			if($res->num_rows > 0){
				echo '<span style="font-size: 20px;font-weight: bold;">Profile of blogger '.$search.'</span><br><br>';
  ...
  if($db){
    ...
    //$row_email=$row['email'];
    //echo $row_email."aa";
    $s="select * from add_blog where email='$row_email'";
    $res=$conn->query($s);
    $cnt=10;
    $idxx=0;
    if($res->num_rows > 0 && $cnt>0) {
      while($row = $res->fetch_assoc()){
        ...
        //echo $row["title"];
        echo "<span class=\"d1\"><a class=\"open_profile blogdis\" href=\"res_open_blog.php?pl=".$tempt."&p2=".$tempe."\" method=\"post\">".$row["title"]."</a>"."&nbsp"." written by</span><span ><a class=\"open_profile emaildis\" href=\"res_open_profile.php?pl=".$tempt."&p2=".$tempe."\" method=\"post\"> ".$row['email']."</a></span><br><br><br><br>";
        ...
        if($res4->num_rows>0){
          while($row = $res4->fetch_assoc()){
            if($row['islike']=='1'){
              echo '<a href="res_like_search.php?pl='.$tempt.'&p2='.$tempe.'" method="post"><div ><i onclick="myFunction'.$idxx.'(this)" class="fa fa-thumbs-up2" id="changed'.$idxx.'"></i></div></a>';
              ...
              echo '<a href="res_like_search.php?pl='.$tempt.'&p2='.$tempe.'" method="post"><div ><i onclick="myFunction'.$idxx.'(this)" class="fa fa-thumbs-down2" id="changed'.$idxx.'"></i></div></a>';
              $s5="select * from add_blog where title='$b1'";
                $res5=$conn->query($s5);
                if($res5->num_rows>0){
                  while($row = $res5->fetch_assoc()){
          ...
          echo '<a href="res_like_search.php?pl='.$tempt.'&p2='.$tempe.'" method="post"><div ><i onclick="myFunction'.$idxx.'(this)" class="fa fa-thumbs-up" id="change'.$idxx.'"></i></div></a>';
...
<div id="idm<?php echo $idxx?>" class="modalm">
  ...
  <form class="modalm-content animate" action="res_comments_search.php?pl=<?php echo $tempt ?>" method="post" style = "width:55%; height:61%">
  ...
  //echo "</fieldset></div>";
				...
				$s="select * from admins where name='$search'";
				$res=$conn->query($s);

				if($res->num_rows > 0){
					echo '<span style="font-size: 20px;font-weight: bold;">Profile of admin '.$search.'</span><br><br>';
...
<div id="idm<?php echo $idxx?>" class="modalm">
  ...
  else
  {
    ...
    if(isset($_SESSION['email']))
    $idx=$_SESSION['email'];
    $s="select * from add_blog where title='$search'";
    $res=$conn->query($s);
    $cnt=10;
    $idxx=0;
    if($res->num_rows > 0 && $cnt>0) {
