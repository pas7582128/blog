...
<head>
  ...
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
	...
	if(isset($_SESSION['email'])){
  echo '<li id="a1"><a href="myblogs.php" >My blogs</a></li><li id="a1"><a id="onlink" href="myprofile.php">My profile</a></li>';
    if(isset($_SESSION['admin'])&& $_SESSION['admin']==1){
...
if($rowCount > 0){
    while($row = mysqli_fetch_assoc($result)){
      $postDetails[] = ucfirst($row['title']);
		...
		if($db){
			$idx=$_SESSION['email'];
			$s="select * from bloggers where email='$idx'";
			$res=$conn->query($s);
			...
			$s="select * from admins where email='$idx'";
			$res=$conn->query($s);
