...
<head>
  ...
  $db=mysqli_select_db($conn,'ass5_3rdyear');
    $em=$_SESSION["email"];
    $s="select * from permit where email='$em'";
    $res=$conn->query($s);
    if($res->num_rows > 0){
      while($row = $res->fetch_assoc()){
        if($row['ispermit']==-1){
          echo '<script type="text/javascript">alert("You are not permitted to add blog");window.location.href="home.php";</script>';$flag=0;
        }
      }
    }
	...
	if(isset($_SESSION['email']))
		echo '<li id="a1"><a id="onlink" href="Add_blog.php">Add blog</a></li>';
...
if($rowCount > 0){
	...
	?>
	<form action="res_add_blog.php" method="POST">
