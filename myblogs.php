...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
  ...
  if(isset($_SESSION['email'])){
  echo '<li id="a1"><a id="onlink" href="myblogs.php" >My blogs</a></li><li id="a1"><a href="myprofile.php">My profile</a></li>';
    if(isset($_SESSION['admin'])&& $_SESSION['admin']==1){
  ...
  if($db){
    if(isset($_SESSION['email']))
    $idx=$_SESSION['email'];
    $s="select * from add_blog where email='$idx'";
    $res=$conn->query($s);
    $cnt=100;
    $idxx=0;
    if($res->num_rows > 0 && $cnt>0) {
      while($row = $res->fetch_assoc()){
        ...
        //echo $row["title"];
        echo "<span class=\"d1\"><a class=\"open_profile\" href=\"res_open_blog.php?pl=".$tempt."&p2=".$tempe."\" method=\"post\">".$row["title"]."</a> written by</span><span ><a class=\"open_profile emaildis\" href=\"res_open_profile.php?pl=".$tempt."&p2=".$tempe."\" method=\"post\"> ".$row['email']."</a></span><br><br><br><br>";
        ...
        if($res4->num_rows>0){
          while($row = $res4->fetch_assoc()){
            if($row['islike']=='1'){
              echo '<a href="res_like_myblog.php?pl='.$tempt.'&p2='.$tempe.'" method="post"><div ><i onclick="myFunction'.$idxx.'(this)" class="fa fa-thumbs-up2" id="changed'.$idxx.'"></i></div></a>';
              ...
              echo '<a href="res_like_myblog.php?pl='.$tempt.'&p2='.$tempe.'" method="post"><div ><i onclick="myFunction'.$idxx.'(this)" class="fa fa-thumbs-down2" id="changed'.$idxx.'"></i></div></a>';
              $s5="select * from add_blog where title='$b1'";
                $res5=$conn->query($s5);
                if($res5->num_rows>0){
                  while($row = $res5->fetch_assoc()){
          ...
          echo '<a href="res_like_myblog.php?pl='.$tempt.'&p2='.$tempe.'" method="post"><div ><i onclick="myFunction'.$idxx.'(this)" class="fa fa-thumbs-up" id="change'.$idxx.'"></i></div></a>';
...
<div id="idm<?php echo $idxx?>" class="modalm">
  ...
  <form class="modalm-content animate" action="res_comments_myblog.php?pl=<?php echo $tempt ?>" method="post" style = "width:55%; height:61%">
