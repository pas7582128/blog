...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
  ...
  if(isset($_SESSION['email'])){
	...
	if($db){
		echo '<span class="blogdis">My notifications</span><br><br>';
		$query = "SELECT * FROM notify where reciever='$email2' order by id desc";
...
if($rowCount > 0){
    while($row = mysqli_fetch_assoc($result)){
			...
			if($row['lorc']==1){
          echo '<span class="daa"><B>'.$row['sender'].'</B> liked your blog <B>'.$row['bname'].'</B> on <B>'.date('jS M Y', strtotime($row['date'])).'</B></span><br><br>';
			}
			else if($row['lorc']==2){
          echo '<span class="daa"><B>'.$row['sender'].'</B> disliked your blog <B>'.$row['bname'].'</B> on <B>'.date('jS M Y', strtotime($row['date'])).'</B></span><br><br>';
			}
			else if($row['lorc']==-1){
          echo '<span class="daa"><B>'.$row['sender'].'</B> commented on your blog <B>'.$row['bname'].'</B> on <B>'.date('jS M Y', strtotime($row['date'])).'</B></span><br><br>';
			}
			echo "</div>";
	}
}
  ...
  echo "<br>";
  echo "<span class=\"daa\">No notifications</span>";
}
	}
	else
	{
